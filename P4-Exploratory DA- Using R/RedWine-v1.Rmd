...
ggcorr(rw_subset,label=TRUE,label_size = 3,nbreaks=5,low = "steelblue", mid = "grey96", high = "darkred",hjust = 0.75, size = 3)
...
ggplot(aes(y=rw$total.sulfur.dioxide,x=rw$quality,group=quality),data=rw)+
  ...
  geom_boxplot(aes(color=factor(rw$quality)))+ylim(0,100)+
...
ggplot(aes(x=density,y=alcohol),data=rw)+
   geom_point(aes(color=factor(quality)),stat='summary',fun.y=mean)+
...
rw_col_subset <- rw %>% select(-total.acidity,-bound.sulfur.dioxide,-prop_freebound_SO2,-sulphates_mg,-pH_bucket)
rw$quality <- as.numeric(rw$quality)

all_model <-lm(quality ~ fixed.acidity+volatile.acidity+citric.acid+
              residual.sugar+chlorides+free.sulfur.dioxide+total.sulfur.dioxide+density+pH+sulphates+alcohol,data = rw_col_subset)
